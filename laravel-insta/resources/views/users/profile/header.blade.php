<div class="row">
    <div class="col-4">
        {{-- Check if the user has an avatar/profile photo --}}
        @if ($user->avatar)
        {{-- Display the avatar if available --}}
            <img src="{{ $user->avatar }}" alt="{{ $user->name }}" class="img-thumbnail rounded-circle d-block mx-auto avatar-lg">
        @else
        {{-- If the avatar is not available, display fontawesome instead --}}
    ...
    <div class="col-8">
        <div class="row mb-3">
            ...
            <div class="col-auto p-2">
                {{-- Check if the logged-in user is equal to the user being search --}}
                @if (Auth::user()->id === $user->id)
                    {{-- Display edit button if equal --}}
